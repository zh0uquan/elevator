...
impl ElevatorData {
    pub fn enqueue(&mut self, floor: u8) {
    ...
    fn next_in_current_direction(&mut self) -> Option<u8> {
    ...
    pub fn next(&mut self) -> Option<u8> {
        if let Some(floor) = self.next_in_current_direction() {
            return Some(floor);
        }
        self.direction_up = !self.direction_up;
        self.next_in_current_direction()
    }
}
